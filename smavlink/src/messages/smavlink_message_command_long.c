/* This file has been autogenerated by Ivory
 * Compiler version  0.1.0.0
 */
#include "smavlink_message_command_long.h"
void smavlink_send_command_long(struct command_long_msg* n_var0,
                                struct smavlink_out_channel* n_var1,
                                struct smavlink_system* n_var2)
{
    uint8_t n_local0[33U] = {0};
    uint8_t* n_ref1 = n_local0;
    float n_deref2 = *&n_var0->param1;
    
    smavlink_pack_float((uint8_t*) n_ref1, 0U, n_deref2);
    
    float n_deref3 = *&n_var0->param2;
    
    smavlink_pack_float((uint8_t*) n_ref1, 4U, n_deref3);
    
    float n_deref4 = *&n_var0->param3;
    
    smavlink_pack_float((uint8_t*) n_ref1, 8U, n_deref4);
    
    float n_deref5 = *&n_var0->param4;
    
    smavlink_pack_float((uint8_t*) n_ref1, 12U, n_deref5);
    
    float n_deref6 = *&n_var0->param5;
    
    smavlink_pack_float((uint8_t*) n_ref1, 16U, n_deref6);
    
    float n_deref7 = *&n_var0->param6;
    
    smavlink_pack_float((uint8_t*) n_ref1, 20U, n_deref7);
    
    float n_deref8 = *&n_var0->param7;
    
    smavlink_pack_float((uint8_t*) n_ref1, 24U, n_deref8);
    
    uint16_t n_deref9 = *&n_var0->command;
    
    smavlink_pack_uint16_t((uint8_t*) n_ref1, 28U, n_deref9);
    
    uint8_t n_deref10 = *&n_var0->target_system;
    
    smavlink_pack_uint8_t((uint8_t*) n_ref1, 30U, n_deref10);
    
    uint8_t n_deref11 = *&n_var0->target_component;
    
    smavlink_pack_uint8_t((uint8_t*) n_ref1, 31U, n_deref11);
    
    uint8_t n_deref12 = *&n_var0->confirmation;
    
    smavlink_pack_uint8_t((uint8_t*) n_ref1, 32U, n_deref12);
    smavlink_send_ivory(n_var1, n_var2, 76U, (uint8_t*) n_ref1, 33U, 152U);
    return;
}
void smavlink_unpack_command_long(struct command_long_msg* n_var0, const
                                  uint8_t** n_var1)
{
    float n_r0 = smavlink_unpack_float(n_var1, 0U);
    
    *&n_var0->param1 = n_r0;
    
    float n_r1 = smavlink_unpack_float(n_var1, 4U);
    
    *&n_var0->param2 = n_r1;
    
    float n_r2 = smavlink_unpack_float(n_var1, 8U);
    
    *&n_var0->param3 = n_r2;
    
    float n_r3 = smavlink_unpack_float(n_var1, 12U);
    
    *&n_var0->param4 = n_r3;
    
    float n_r4 = smavlink_unpack_float(n_var1, 16U);
    
    *&n_var0->param5 = n_r4;
    
    float n_r5 = smavlink_unpack_float(n_var1, 20U);
    
    *&n_var0->param6 = n_r5;
    
    float n_r6 = smavlink_unpack_float(n_var1, 24U);
    
    *&n_var0->param7 = n_r6;
    
    uint16_t n_r7 = smavlink_unpack_uint16_t(n_var1, 28U);
    
    *&n_var0->command = n_r7;
    
    uint8_t n_r8 = smavlink_unpack_uint8_t(n_var1, 30U);
    
    *&n_var0->target_system = n_r8;
    
    uint8_t n_r9 = smavlink_unpack_uint8_t(n_var1, 31U);
    
    *&n_var0->target_component = n_r9;
    
    uint8_t n_r10 = smavlink_unpack_uint8_t(n_var1, 32U);
    
    *&n_var0->confirmation = n_r10;
}

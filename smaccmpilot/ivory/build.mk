...
$(HDRS) $(SRCS):
	$(SANDBOX)/bin/$(GEN) \
    --src-dir=$(SRCDIR) \
    --include-dir=$(INCDIR) $(IVORY-OPTS)
    #  \
    # --cfg=true \
    # --cfg-proc=$(SRCDIR)/pid_stabilize.c

# Build the binary to generate the code.
.PRECIOUS: $(EXEC)
$(EXEC):
	cabal-dev -s $(SANDBOX) install \
    --builddir=$(SMACCMDIR)/ivory $(SMACCMDIR)/ivory

CLEAN     += $(HDRS) $(SRCS)
VERYCLEAN += $(SMACCMDIR)/ivory/dist $(SMACCMDIR)/ivory/cabal-dev

...
app topx4 = do
  ...
  monitor "sensorsender" $ do
    ...
    -- gyroSender (snd g_sample)  uartout
    accelSender (snd a_sample)  uartout
  ...
  case px4platform_can px4platform of
    ...
    Just can -> do
      ...
      fragmentSenderBlind (snd g_sample) 0x001 False canReq (Proxy :: Proxy 24) -- 200Hz, 5 fragments
...
baro_mag_manager :: (e -> PX4Platform)
                 -> Tower e ( ChanOutput (Struct "barometer_sample")

...
#
# modified for accel and mag calibration
# Michal Podhradsky, 20 March 2017
...
class Mag(object):
    ...
    def display(self):
        if self.errormsg:
            return self.errormsg
        return ("%3.3f 66 IMU_MAG_RAW %d %d %d" %
...
class Accel(object):
    ...
    def display(self):
        if self.errormsg:
            return self.errormsg
        return ("%3.3f 66 IMU_ACCEL_RAW %d %d %d" %
            (self.t/1000000.0, self.x, self.y, self.z))
...
class Gyro(object):
    ...
    def display(self):
        if self.errormsg:
            return self.errormsg
        return ("%3.3f 66 IMU_GYRO_RAW %d %d %d" %
            (self.t/1000000.0, self.x, self.y, self.z))

...
-- XXX these no longer depend on PlatformClock.
-- we should just make these Integer constants and then

getFreqSysClk :: (GetAlloc eff ~ Scope s, PlatformClock p)
...
getFreqHClk :: (GetAlloc eff ~ Scope s, PlatformClock p)
            => Proxy p -> Ivory eff Uint32
getFreqHClk _ = return (fromIntegral (div sysClkHz 1))

getFreqPClk1 :: (GetAlloc eff ~ Scope s, PlatformClock p)
             => Proxy p -> Ivory eff Uint32
getFreqPClk1 _ = return (fromIntegral (div sysClkHz 4))

getFreqPClk2 :: (GetAlloc eff ~ Scope s, PlatformClock p)
             => Proxy p -> Ivory eff Uint32
getFreqPClk2 _ = return (fromIntegral (div sysClkHz 2))

data PClk = PClk1 | PClk2

getFreqPClk :: (GetAlloc eff ~ Scope s, PlatformClock p) => Proxy p -> PClk -> Ivory eff Uint32

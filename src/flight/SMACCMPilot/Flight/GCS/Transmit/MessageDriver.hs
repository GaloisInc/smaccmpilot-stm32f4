...
mkSendHeartbeat =
  ...
  $ \cl seqNum sendStruct comm_monitor -> body $ do
  ...
  -- monitor is bit 8 (0 for alarm)
  custom_field_monitor    = 256
...
mkSendNavLaw = proc "gcs_transmit_send_nav_law" $
  ...
  msg <- local (istruct
    ...
    , SN.heading_set       .= ival (castWith 0 (heading_setpt * 180.0 * 100.0 / pi))

...
gcsTower :: (SingI n0, SingI n1)
         ...
         -> [Param PortPair]
         -> Tower p ()
gcsTower name opts istream ostream fm armed sens pos ctl motor params =
  void (gcsTowerAux name opts istream ostream fm armed sens pos ctl motor params)
...
gcsTowerHil :: (SingI n0, SingI n1)
         ...
         -> [Param PortPair]
         -> Tower p ()
gcsTowerHil name opts istream ostream fm armed ctl motor sensors params = do
  sensors_state <- stateProxy (snk sensors)
  position      <- dataport
  hil <- gcs fm armed sensors_state (snk position) ctl motor params
...
gcsTowerAux name opts istream ostream fm armed sens pos ctl motor params = do
  ...
  task (named "gcsReceiveTask") $
    gcsReceiveTask decToGcsRxRcv (src streamrate) (src datarate) (src hil)
      (src fm) (src armed) (src param_req) params
